...
curl https://cockpit-1.tail151af.ts.net/api/dashboard

# writes really are refused from outside
curl -X POST https://cockpit-1.tail151af.ts.net/api/approvals/apr-001/approve \
  -H "content-type: application/json" -d '{"scope":"test"}'
# -> 403 "Writes are local-only."

# the rules are public, not a description of rules
curl https://cockpit-1.tail151af.ts.net/moderation

# what an agent is told on arrival
curl https://cockpit-1.tail151af.ts.net/llms.txt
```
