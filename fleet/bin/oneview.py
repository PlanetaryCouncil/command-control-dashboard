...
/* Talking to CLAUDE. A different thing from talking to the board, and a
   ...
   back with it. */
/* Send stays beside the box, on the right, full height. Marsita, 2026-09-18:
   "you did something funny with send image size... I prefer it is on the
   right". Moving it up into the control row changed its shape, and it was
   fine where it was: it is the button you came here to press and it belongs
   against the thing you just typed. */
#tell{flex:none;display:flex;gap:6px;align-items:flex-end;padding:5px 7px;
  border-top:1px solid var(--border);background:var(--raised);}
/* The reload row sits ABOVE the form, indented to the same 7px as the box, so
   the pill's left edge lines up with the text you are writing. It holds only
   the pill, and collapses when the pill is hidden -- an empty strip above the
   box is a gap that never explains itself. */
.tellhead{flex:none;display:none;align-items:center;padding:5px 7px 0;}
.tellhead:has(#rebtn[data-stale="1"]){display:flex;}
#termpane[data-open="0"] .tellhead{display:none;}
...
.tellhead #rebtn{display:inline-flex;align-items:center;gap:5px;
...
.tellhead #rebtn svg{width:11px;height:11px;fill:none;stroke:currentColor;
  stroke-width:2.2;stroke-linecap:round;stroke-linejoin:round;
  transition:transform .5s cubic-bezier(.3,1.6,.4,1);}
.tellhead #rebtn:hover{color:#0d0d0d;background:var(--good);border-color:var(--good);}
.tellhead #rebtn:hover svg{transform:rotate(180deg);}
/* Pressed state, so the click is felt as well as seen. */
.tellhead #rebtn:active{transform:translateY(1px);}
.tellhead #rebtn:focus-visible{outline:2px solid var(--good);outline-offset:2px;}
/* The label and the glyph are decoration INSIDE the button; a click landing on
   either must still be a click on the button. */
.tellhead #rebtn > *{pointer-events:none;}
...
.tellhead #rebtn{display:none;}
.tellhead #rebtn[data-stale="1"]{display:inline-flex;}
/* It only ever appears when it is the thing to press, so it does not have to
   be shy about it. */
.tellhead #rebtn:hover{color:#0d0d0d;background:#ffc93c;border-color:#ffc93c;}
...
.tellhead #rebtn[data-stale="1"]{color:#1a1200;background:var(--warning);
...
.tellhead #rebtn[data-stale="1"]:hover{background:#ffd469;border-color:#fff;}
.tellhead #rebtn[data-stale="1"] svg{animation:respin 1.6s linear infinite;}
...
@media (prefers-reduced-motion:reduce){
  .tellhead #rebtn[data-stale="1"] svg{animation:none;}
  .tellhead #rebtn[data-stale="1"]{animation:none;}
  .tellhead #rebtn svg{transition:none;}
}
